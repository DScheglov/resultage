...
describe('AsyncResult', () => {
  ...
  describe('map', () => {
    it('should map an AsyncOk', async () =>
      expect(
        pipe(
          1,
          asyncOk,
    ...
    it('should map an AsyncOk to the same AsyncOk with identity', () =>
      expect(pipe(1, asyncOk, M.thenMap(identity))).resolves.toEqual(ok(1)));

    it('should map twice with the same result as mapping once with composition', async () => {
      const f = (x: number) => x + 1;
      const g = (x: number) => x * 2;
      const value1 = await pipe(1, asyncOk, M.thenMap(f), M.thenMap(g));

      const value2 = await pipe(1, asyncOk, M.thenMap(compose(g, f)));
  ...
  describe('mapErr', () => {
    it('should not map an AsyncOk', async () =>
      expect(
        pipe(
          1,
          asyncOk,
    ...
    it('should map an AsyncErr to the same AsyncErr with identity', () =>
      expect(pipe(1, asyncErr, M.thenMapErr(identity))).resolves.toEqual(
        err(1),
      ));
    ...
    it('should map twice with the same result as mapping once with composition', async () => {
      ...
      const value1 = await pipe(1, asyncErr, M.thenMapErr(f), M.thenMapErr(g));

      const value2 = await pipe(1, asyncErr, M.thenMapErr(compose(g, f)));
  ...
  describe('flip', () => {
    it('should flip an Ok<Promise<T>> to Promise<Ok<T>>', async () =>
      expect(pipe(ok(resolved(1)), M.flip)).resolves.toEqual(ok(1)));

    it('should flip an Ok<T> to Promise<Ok<T>>', async () =>
      expect(pipe(ok(1), M.flip)).resolves.toEqual(ok(1)));

    it('should flip an Err<Promise<E>> to Promise<Err<E>>', async () =>
      expect(pipe(err(resolved(1)), M.flip)).resolves.toEqual(err(1)));

    it('should flip an Err<E> to Promise<Err<E>>', async () =>
      expect(pipe(err(1), M.flip)).resolves.toEqual(err(1)));
  });

  describe('chain', () => {
    it('should chain an AsyncOk', async () =>
      expect(
        pipe(
          1,
          asyncOk,
    ...
    it('should chain an AsyncOk to the same AsyncOk with asyncOK', () =>
      expect(pipe(1, asyncOk, M.thenChain(asyncOk))).resolves.toEqual(ok(1)));
    ...
    it('should chain twice with the same result as chaining with chained result (sync)', async () => {
      ...
      const value1 = await pipe(1, asyncOk, M.thenChain(f), M.thenChain(g));

      const value2 = await pipe(
        1,
        asyncOk,
        M.thenChain((result) => pipe(result, f, M.from, M.thenChain(g))),
      );
    ...
    it('should chain twice with the same result as chaining with sync chained result (sync)', async () => {
      ...
      const value2 = await pipe(
        1,
        asyncOk,
        M.thenChain((result) => pipe(result, f, R.chain(g))),
      );
    ...
    it('should chain twice with the same result as chaining with chained result (async)', async () => {
      ...
      const value2 = await pipe(
        1,
        asyncOk,
        M.thenChain((result) => pipe(result, f, M.thenChain(g))),
      );
    ...
    it('should chain ok to err (sync)', async () => {
      ...
      const value1 = await pipe(1, asyncOk, M.thenChain(fn));
      const value2 = await pipe(-1, asyncOk, M.thenChain(fn));
  ...
  describe('chainErr', () => {
    it('should not chain an AsyncOk', async () =>
      expect(
        pipe(
          1,
          asyncOk,
    ...
    it('should chain an AsyncErr to the same AsyncErr with asyncErr', () =>
      expect(pipe(1, asyncErr, M.thenChainErr(asyncErr))).resolves.toEqual(
        err(1),
      ));
    ...
    it('should chain twice with the same result as chaining with chained result (sync)', async () => {
      ...
      const value2 = await pipe(
        1,
        asyncErr,
        M.thenChainErr((result) => pipe(result, f, M.from, M.thenChainErr(g))),
      );
    ...
    it('should chain twice with the same result as chaining with sync chained result (sync)', async () => {
      ...
      const value2 = await pipe(
        1,
        asyncErr,
        M.thenChainErr((result) => pipe(result, f, R.chainErr(g))),
      );
    ...
    it('should chain twice with the same result as chaining with chained result (async)', async () => {
      ...
      const value2 = await pipe(
        1,
        asyncErr,
        M.thenChainErr((result) => pipe(result, f, M.thenChainErr(g))),
      );
    ...
    it('should chain err to ok (sync)', async () => {
      const fn = (x: number) => (x > 0 ? ok(x) : err('INVALID_VALUE' as const));
      const value1 = await pipe(1, asyncErr, M.thenChainErr(fn));
      const value2 = await pipe(-1, asyncErr, M.thenChainErr(fn));
      expect(value1).toEqual(ok(1));
      expect(value2).toEqual(err('INVALID_VALUE'));
    });
  ...
  describe('unwrap', () => {
    it('should unpack an AsyncOk', async () =>
      expect(pipe(1, asyncOk, M.thenUnwrap)).resolves.toBe(1));
  ...
  describe('unwrapOr', () => {
    it('should unpack an AsyncOk', async () =>
      expect(pipe(1, asyncOk, M.thenUnwrapOr('bar'))).resolves.toBe(1));

    it('should return the fallback for an AsyncErr', async () =>
      expect(pipe(1, asyncErr, M.thenUnwrapOr('bar'))).resolves.toBe('bar'));
  });
  ...
  describe('unwrapOrReject', () => {
    it('should unpack an AsyncOk', async () =>
      expect(pipe(1, asyncOk, M.thenUnwrapOrReject)).resolves.toBe(1));

    it('should unpack an AsyncErr', async () =>
      expect(pipe(1, asyncErr, M.thenUnwrapOrReject)).rejects.toBe(1));
  });

  describe('unwrapErr', () => {
    it('should unpack an AsyncErr', async () =>
      expect(pipe(1, asyncErr, M.thenUnwrapErr)).resolves.toBe(1));
  ...
  describe('unwrapErrOr', () => {
    it('should unpack an AsyncErr', async () =>
      expect(pipe(1, asyncErr, M.thenUnwrapErrOr('bar'))).resolves.toBe(1));

    it('should return the fallback for an AsyncOk', async () =>
      expect(pipe(1, asyncOk, M.thenUnwrapErrOr('bar'))).resolves.toBe('bar'));
  });
  ...
  describe('tap', () => {
    it('should tap an AsyncOk', async () => {
      const fn = jest.fn();
      await pipe(1, asyncOk, M.thenTap(fn));
      expect(fn).toHaveBeenCalledWith(1);
    });

    it('should not tap an AsyncErr', async () => {
      const fn = jest.fn();
      await pipe(1, asyncErr, M.thenTap(fn));
  ...
  describe('tapErr', () => {
    it('should not tap an AsyncOk', async () => {
      const fn = jest.fn();
      await pipe(1, asyncOk, M.thenTapErr(fn));
      expect(fn).not.toHaveBeenCalled();
    });

    it('should tap an AsyncErr', async () => {
      const fn = jest.fn();
      await pipe(1, asyncErr, M.thenTapErr(fn));
      expect(fn).toHaveBeenCalledWith(1);
    });
  });
  ...
  describe('tapAndWait', () => {
    ...
    it('should tap an AsyncOk', async () => {
      ...
      const asyncRes = pipe(1, asyncOk, M.thenTapAndWait(fn), M.thenMap(inc));
      await delay();
      expect(fn).toHaveBeenCalledWith(1);
