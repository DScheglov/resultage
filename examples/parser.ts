...
import { Result, err, ok } from '@cardellini/ts-result';
...
const okIfPerson2 = (value: unknown) =>
  okIfObject(value)
    .chain((object) =>
      okIfNotEmptyStr(object.name).chain((name) =>
        okIfInt(object.age).chain((someInt) =>
          okIfPositive(someInt).map((age) => ({ name, age })),
        ),
      ),
    )
    .mapErr(() => 'ERR_NOT_A_PERSON' as const);

const okIfPerson3 = (value: unknown) =>
  okIfObject(value)
    .chain((object) =>
      okIfNotEmptyStr(object.name).chain((name) =>
        okIfInt(object.age)
...
const okIfPerson4 = (value: unknown): Result<Person, 'ERR_NOT_A_PERSON'> =>
  okIfObject(value)
    .chain(({ name, age }) =>
      okIfNotEmptyStr(name).map((validName) => ({ name: validName, age })),
    )
    .chain(({ name, age }) =>
      okIfInt(age)
        .chain(okIfPositive)
        .map((validAge) => ({ name, age: validAge })),
    )
...
const readField =
  ...
  <R extends {}>(object: R): Result<unknown, 'ERR_MISSING_FIELD'> =>
    field in object ? ok((object as any)[field]) : err('ERR_MISSING_FIELD');
...
const validateField =
  <F extends string, T, E>(
    field: F,
    validator: (value: unknown) => Result<T, E>,
  ) =>
  <R extends {}>(
    object: R,
  ): Result<R & Record<F, T>, E | 'ERR_MISSING_FIELD'> =>
    ok(object)
      .chain(readField(field))
      .chain(validator)
      .map((v) => writeField(object, field)(v));

const validateName = validateField('name', okIfNotEmptyStr);
const validateAge = validateField('age', (value) =>
  okIfInt(value).chain(okIfPositive),
);

const okIfPerson5 = (value: unknown): Result<Person, 'ERR_NOT_A_PERSON'> =>
  okIfObject(value)
    .chain((obj) => validateName(obj))
    .chain((obj) => validateAge(obj))
    .mapErr(() => 'ERR_NOT_A_PERSON' as const);
