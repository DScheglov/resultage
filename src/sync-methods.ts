...
export const unwrapOr = <S>(fallback: S) =>
  <E, T>(result: Result<T, E>): T | S => result.unwrapOr(fallback);
...
<E, S>(fallback: (error: E) => S) =>
    <T>(result: Result<T, E>): T | S => result.unwrapOrElse(fallback);

export const unwrapErr = <E, T>(result: Result<T, E>): E => result.unwrapErr();

export const unwrapErrOr = <F>(fallback: F) =>
  <E, T>(result: Result<T, E>): E | F => result.unwrapErrOr(fallback);

export const unwrapErrOrElse =
  <T, F>(fallback: (data: T) => F) =>
    <E>(result: Result<T, E>): E | F => result.unwrapErrOrElse(fallback);
...
export const tap =
  <T>(fn: (data: T) => void) =>
    <E>(result: Result<T, E>): Result<T, E> => result.tap(fn);

export const tapErr =
  <E>(fn: (error: E) => void) =>
    <T>(result: Result<T, E>): Result<T, E> => result.tapErr(fn);
